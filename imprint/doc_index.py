...
def cache_dir() -> str:
    """Index cache location: IMPRINT_INDEX_DIR override, else the roomy E:
    offload drive when connected (keeps the small C: SSD clear), else
    %LOCALAPPDATA%."""
    override = os.environ.get("IMPRINT_INDEX_DIR")
    if override:
        return override
    if os.path.isdir("E:\\"):
        return os.path.join("E:\\", "Imprint")
    base = (os.environ.get("LOCALAPPDATA")
            or os.path.expanduser(r"~\AppData\Local"))
    return os.path.join(base, "Imprint")
